...
public class RelationQueryDao {
    ...
    private String buildCypher(RelationQueryArgs args) {
        ...
        if (args.getStartName().size() == 0 && StringUtils.isBlank(args.getEndName()))
            name = "";
        else if (args.getStartName().size() != 0 && StringUtils.isNotBlank(args.getEndName()))
            name = String.format("WHERE start.name IN %s AND end.name = '%s'", nameList, args.getEndName());
        else if (args.getStartName().size() != 0)
            name = String.format("WHERE start.name IN %s ", nameList);
        else
            name = String.format("WHERE end.name = '%s' ", args.getEndName());
        // 拼接cypherSQL
